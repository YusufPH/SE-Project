    ...
    public void addPartBtn(ActionEvent event)   //add Part to Vehicle button
    {
            try {
                if(!namefield.equals("") && !costfield.equals(0) && !quanfield.equals(0) && !idfield.equals(0)) //make sure the fields arent empty
                {
                        ...
                        alert.showAndWait();
                        String stmt = "INSERT INTO Parts (ID, Name, Description, StockQuantity, Cost) VALUES ('" + Integer.parseInt(idfield.getText()) + "','" + namefield.getText() + "', '" + descfield.getText() + "', '" + Integer.parseInt(quanfield.getText()) + "','" + Integer.parseInt(costfield.getText()) +"')";     //statement to insert
